...
public:
    ...
    Q_SLOT void make_clicked()
    {
        emit create_project((int)settings.type);
    }

    Q_SLOT void load_clicked()
    {
        emit load_project((int)settings.type);
    }

public:
    Q_SIGNAL void MouseLeaveSignal();
    Q_SIGNAL void MouseEnterSignal();
    Q_SIGNAL void create_project(int type);
    Q_SIGNAL void load_project(int type);
};
