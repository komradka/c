...
error initial_approximation_builder::construct_rate_initial_approximation(bool is_simple_ia, const thread_info &thr_info)
{
    if (is_simple_ia)
        return simple_rate_initial_approximation(thr_info);

    thr_info.barrier ();
    return error(OK);
}

error initial_approximation_builder::simple_rate_initial_approximation(const thread_info &thr_info)
{
